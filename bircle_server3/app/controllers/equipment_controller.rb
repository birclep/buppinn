class EquipmentController < ApplicationController
  ...
  before_filter :rec
  def rec
    file = Dir.glob("/home/sit-user-15/rails_test/bircle_server3/recognition/*.jpg").sort_by {|f| File.mtime(f)}.reverse[0]
    puts file
    @equipment_check = EquipmentClass.find_all_by_group_id(session[:group_id])
    if @equipment_check.length > 1
    o, e, s = Open3.capture3("/home/sit-user-15/recognizertest/te " + file +" "+ session[:group_id].to_s)
    @str = o
    @ft = file
   end
  end
  ...
  def index
    Dir.chdir("/home/sit-user-15/rails_test/bircle_server3/recognition/")
    file2 = Dir.glob("*.jpg").sort_by {|f| File.mtime(f)}.reverse[0]
    puts file2
    @ft2 = file2
    @rec_img = Rails.root.join('recognition', @ft2)
    @equipment_class = EquipmentClass.find_all_by_group_id(session[:group_id])
    if @equipment_check.length == 0
      ...
      @equipment = EquipmentClass.find_by_id(@str)
      @test = @equipment.equipment_classes_name
      @equipment_id = @equipment.id
      @brURL = "/equipment/new?id=" + @equipment_id.to_s
    end
